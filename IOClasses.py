...
# Ziqi Yan, 2022-03-27, Complete Todo
#------------------------------------------#
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(file_name[1], 'r') as file:
                for line in file:
                    data = line.strip().split(',')
                    PC.DataProcessor.select_cd(lst_Inventory, int(data[0])).add_track(DC.Track(int(data[1]),data[2],data[3]))
        except Exception as e:
            print('There was a general error!', e, e.__doc__, type(e), sep='\n')
